...
if [ ! -d "$CAFFE2_ROOT/$BUILD_DIR" ]; then
  ...
  IOS_PLATFORM=SIMULATOR BUILD_DIR=$BUILD_DIR INSTALL_DIR="$INSTALL_ROOT_DIR/x86_64" $SCRIPTS_DIR/build_ios_pod.sh
fi
...
## DEVICE
# Build for iOS
BUILD_DIR="build_ios_pod"
if [ ! -d "$CAFFE2_ROOT/$BUILD_DIR" ]; then
  IOS_PLATFORM=OS BUILD_DIR=$BUILD_DIR INSTALL_DIR="$INSTALL_ROOT_DIR/arm" $SCRIPTS_DIR/build_ios_pod.sh
fi
cd "$CAFFE2_ROOT/$BUILD_DIR"
...
lipo -create \
    "$CAFFE2_ROOT/build_ios_pod/libCAFFE2_NNPACK.a" \
